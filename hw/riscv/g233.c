...
{
    bool msimode = aia_type == G233_AIA_TYPE_APLIC_IMSIC;

    return riscv_is_kvm_aia_aplic_imsic(msimode);
}

static bool virt_use_emulated_aplic(RISCVG233AIAType aia_type)
{
    bool msimode = aia_type == G233_AIA_TYPE_APLIC_IMSIC;
...
static void virt_flash_create(RISCVG233State *s)
{
...
static void virt_flash_map(RISCVG233State *s,
                           MemoryRegion *sysmem)
{
...
static void create_pcie_irq_map(RISCVG233State *s, void *fdt, char *nodename,
...
static void create_fdt_one_imsic(RISCVG233State *s, hwaddr base_addr,
...
static void create_fdt_imsic(RISCVG233State *s,
                             uint32_t *phandle, uint32_t *intc_phandles,
...
static void create_fdt_one_aplic(RISCVG233State *s, int socket,
                                 ...
                                 bool m_mode, int num_harts)
{
    ...
    if (s->aia_type == G233_AIA_TYPE_APLIC) {
...
static void create_fdt_socket_aplic(RISCVG233State *s,
                                    int socket,
...
static void create_fdt_pmu(RISCVG233State *s)
{
...
static void create_fdt_sockets(RISCVG233State *s,
                               uint32_t *phandle,
...
static void create_fdt_virtio(RISCVG233State *s, uint32_t irq_virtio_phandle)
{
    int i;
    MachineState *ms = MACHINE(s);
...
static bool g233_is_iommu_sys_enabled(RISCVG233State *s)
{
    return s->iommu_sys == ON_OFF_AUTO_ON;
}

static void create_fdt_pcie(RISCVG233State *s,
                            uint32_t irq_pcie_phandle,
                            uint32_t msi_pcie_phandle,
...
static void create_fdt_reset(RISCVG233State *s, uint32_t *phandle)
{
    char *name;
...
static void create_fdt_fw_cfg(RISCVG233State *s)
{
    MachineState *ms = MACHINE(s);
...
static void create_fdt_virtio_iommu(RISCVG233State *s, uint16_t bdf)
{
...
static void create_fdt_iommu_sys(RISCVG233State *s, uint32_t irq_chip,
                                 uint32_t msi_phandle,
                                 uint32_t *iommu_sys_phandle)
{
...
static void create_fdt_iommu(RISCVG233State *s, uint16_t bdf)
{
...
static DeviceState *virt_create_aia(RISCVG233AIAType aia_type, int aia_guests,
...
static void create_platform_bus(RISCVG233State *s, DeviceState *irqchip)
{
    DeviceState *dev;
    SysBusDevice *sysbus;
    int i;
...
static void virt_build_smbios(RISCVG233State *s)
{
...
static void virt_machine_init(MachineState *machine)
{
    ...
    {
        DeviceState *gpio_dev = qdev_new(TYPE_G233_GPIO);
        sysbus_realize_and_unref(SYS_BUS_DEVICE(gpio_dev), &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(gpio_dev), 0,
    ...
    /* G233 PWM controller */
    {
        DeviceState *pwm_dev = qdev_new(TYPE_G233_PWM);
        sysbus_realize_and_unref(SYS_BUS_DEVICE(pwm_dev), &error_fatal);
        sysbus_mmio_map(SYS_BUS_DEVICE(pwm_dev), 0,
                        s->memmap[VIRT_PWM].base);
        sysbus_connect_irq(SYS_BUS_DEVICE(pwm_dev), 0,
...
static char *virt_get_aia(Object *obj, Error **errp)
{
    ...
    switch (s->aia_type) {
    case G233_AIA_TYPE_APLIC:
        val = "aplic";
        break;
    case G233_AIA_TYPE_APLIC_IMSIC:
        val = "aplic-imsic";
        break;
    default:
        val = "none";
...
static void virt_set_aia(Object *obj, const char *val, Error **errp)
{
    ...
    if (!strcmp(val, "none")) {
        s->aia_type = G233_AIA_TYPE_NONE;
    } else if (!strcmp(val, "aplic")) {
        s->aia_type = G233_AIA_TYPE_APLIC;
    } else if (!strcmp(val, "aplic-imsic")) {
        s->aia_type = G233_AIA_TYPE_APLIC_IMSIC;
    } else {
...
{
    RISCVG233State *s = RISCV_G233_MACHINE(hotplug_dev);
